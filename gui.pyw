...
def speeding():
    B2.flash()
    text.delete(1.0,END)
    arg = "speeding.py {1},{2},{3},{4} {0}".format(E2.get(),V21.get(),V22.get(),CheckVar21.get(),CheckVar22.get())
...
def marshaling():
    B1.flash()
    text.delete(1.0,END)
    arg = "marshaling.py {3},{4},{5},{1},{2} {0}".format(E1.get(),CheckVar11.get(),CheckVar12.get(),V11.get(),V12.get(),CheckVar13.get())
    p = Popen(arg , stdout=PIPE, shell=True, universal_newlines=True) # to run on windows need to add "python"
...
C21 = Checkbutton(top, text = "show all point of competitor in the restricted zone", variable = CheckVar21, onvalue = "yes", offvalue = "no", width = 40, font=("Ariel", "12"))
